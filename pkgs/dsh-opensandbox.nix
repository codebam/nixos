...
stdenvNoCC.mkDerivation {
  ...
  # @codebam/dsh-opensandbox 0.2.1 is the published artifact of the
  # `security/mount-boundary` branch: the mount-root boundary (`mountRootFor`
  # no longer binds an arbitrary host directory), per-session workspace roots
  # for dsh web, the mount-fenced ctx.fs backend, and the human
  # `/directory-add` family. Fetching the npm tarball means the flake runs
  # exactly what npm consumers install; the GitHub branch remains the
  # reviewable source until it is merged upstream.
  src = fetchzip {
    url = "https://registry.npmjs.org/@codebam/dsh-opensandbox/-/dsh-opensandbox-0.2.1.tgz";
    hash = "sha256-BeA+lemZDfdaULtRZu88dazEijlCL/iYT9HIpLBHdPA=";
  };
