...
let
  ...
  #
  # True since 2026-08-25. Note what that settled, because it cannot be
  # unsettled: the first serve issues a Let's Encrypt certificate, that
  # certificate is published to Certificate Transparency logs, and the hostname
  # is enumerable from those logs within minutes. Setting this back to false
  # closes the door but does not unpublish the address -- from here on, assume
  # strangers know the endpoint exists and that llm-proxy's bearer check is the
  # only thing between them and the card.
  #
  # The ExecCondition on llm-funnel-up still gates on the window, so activation
  # outside 03:00-11:00 skips rather than publishes; the first exposure is the
  # next 03:00 trigger.
  funnelAtBoot = true;
...
in
{
  ...
  systemd.services.llm-funnel-up = {
    ...
    after = [
      "llm-proxy.service"
      "tailscaled.service"
      # Do not publish before the slot count has been raised and the model has
      # been verified to fit -- llm-window-up reverts to single-slot on a
      # bad fit, and the funnel should open onto whatever it settled on.
      "llm-window-up.service"
    ];
  ...
  # there. Kept separate from the funnel units, and its timers run regardless
  # of funnelAtBoot, because the profile is a property of the card rather than
  # of whether the public door is open -- and because the down side must run
  # even if the up side never did.
  systemd.services.llm-window-up = {
    ...
    serviceConfig = {
      Type = "oneshot";
      RemainAfterExit = true;
      # Same skip-not-fail contract as llm-funnel-up, so Persistent=true on the
      # timer cannot replay an 03:00 trigger into a 13:00 load with the
      # compositor up.
      ExecCondition = [ "${inWindow}" ];
  ...
  systemd.services.llm-window-down = {
    ...
    # Close the public door before taking the model away, so the last visitors
    # of the window get "outside service hours" rather than a connection error
    # into a stopped server.
    after = [ "llm-funnel-down.service" ];
    serviceConfig = {
      Type = "oneshot";
  ...
  systemd.timers.llm-window-down = {
    wantedBy = [ "timers.target" ];
    timerConfig = {
      # Same minute as llm-funnel-down; the After= on the unit is what orders
      # them, not the clock.
