...
in
{
  home = {
    ...
    activation = {
      ...
      # One-shot cleanup after dropping the CrofAI provider from the managed
      # fragment above: the merge only adds and overrides keys, so the route
      # an earlier activation wrote into the live document would linger.
      # Ordered after dshSettings, which validates and rebuilds the document;
      # `del` on the missing key is a no-op.
      dshCrofCleanup = lib.hm.dag.entryAfter [ "writeBoundary" "dshSettings" ] ''
